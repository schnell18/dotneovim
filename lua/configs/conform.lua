local options = {
  ...
  format_on_save = function(bufnr)
    ...
    local root_dir = vim.fs.dirname(vim.fs.find(root_patterns, { upward = true })
      [1])
    local exists = root_dir and
        vim.fn.filereadable(vim.fs.joinpath(root_dir, ".no-fmt-on-save"))
    if exists == 1 then
      return false
    end
    return {
